...
int Granular3DAudioProcessor::myState = 0;
...
//bufferToFill이라는 이름의 AudioSourceChannelInfo는 standalone프로그램 일 경우 흔히 쓰이는 변수명임
...
void Granular3DAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    if (myState == Playing)
    {
        for (int channel = 0; channel < totalNumInputChannels; ++channel)
        {
            ...
            // Generate and add white noise to each sample
            for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
            {
                // Generate a random value between -1 and 1
                const float noiseValue = 0.05 * (juce::Random::getSystemRandom().nextFloat() * 2.0f - 1.0f);
...
void Granular3DAudioProcessor::changeListenerCallback (juce::ChangeBroadcaster* source)
{
    if (source == &transportSource)
    {
        ...
        else if ((myState == Stopping) || (myState == Playing))
            myState = Stopped;
        else if (Pausing == myState)
            myState = Paused;
    }

    //Granular3DAudioProcessorEditor::changeListenerCallback();
//        if (source == &transportSource)
//        {
//            if (transportSource.isPlaying())
//                Granular3DAudioProcessorEditor::changeState (Granular3DAudioProcessorEditor::Playing);
//            else
//                changeState (Granular3DAudioProcessorEditor::Stopped);
//        }
}
